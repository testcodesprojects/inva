...
def write_id_names_file(names: List[str], filename: str) -> None:
    """Write factor level names in R-INLA's inla.writeLines binary format.
...
# ---------------------------------------------------------------------------
# R-compatible number formatting (mirrors R's cat() with options(digits))
# ---------------------------------------------------------------------------
_R_DIGITS: int = 7  # R's default: options(digits = 7)


def get_r_digits() -> int:
    """Return the current significant-digits setting used by _format_number."""
    return _R_DIGITS


def set_r_digits(n: int) -> None:
    """Set the significant-digits setting used by _format_number.

    This mirrors R's ``options(digits = n)``.  The default is 7 (R's own
    default).  Comparison notebooks that source R code with a non-default
    ``options(digits = ...)`` should call this so that pyINLA produces
    identically formatted Model.ini files.
    """
    global _R_DIGITS
    if not (1 <= n <= 22):
...
def _format_number(val: Any) -> str:
    """Format *val* the same way R's ``cat()`` does.

    Uses Python's ``g`` conversion which, like R, outputs *_R_DIGITS*
    significant digits and switches to scientific notation for very
...
def inla_write_hyper(hyper: Optional[List[Dict[str, Any]]],
    ...
    for k, hk in enumerate(hyper, start=1):
        ...
        elif tmp_prior.lower().startswith("expression:"):
            # Expression priors: write as-is, do NOT trim (matches R-INLA behaviour)
...
def inla_ffield_section(file: str,
    ...
    # Note: bym2 requires constr=True (R-INLA does not support constr=FALSE for bym2)
    # R-INLA's Model.ini uses constraint=0 for bym2 because it handles constraints via extraconstraint file
    ...
    if not random_spec.get("hyper"):
        default_hyper = _default_random_hyper(random_spec.get("model"))
        if not default_hyper:
            # For models with adaptive PC priors (bym2, rw2diid), we MUST use model_props hyper
            # because the PC prior on phi requires table generation based on the graph.
            # Other models (like bym) don't need this - they use standard loggamma priors.
            if inla_one_of(random_spec.get("model"), ["bym2", "rw2diid"]):
                hyper_dict = prop.get("hyper", {})
                if hyper_dict and isinstance(hyper_dict, dict):
                    # Convert from dict format {'theta1': {...}, 'theta2': {...}} to list format
                    default_hyper = list(hyper_dict.values())
        random_spec["hyper"] = default_hyper
...
def inla_inla_section(file: str, inla_spec: Dict[str, Any], data_dir: str, inla_mode: str) -> None:
    ...
    if merged.get("tolerance_step") is None:
        merged["tolerance_step"] = tol / 2000.0
