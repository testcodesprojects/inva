...
Windows: Uses https://inla.r-inla-download.org/Windows-builds/FILES listing
"""
...
class BinaryManager:
    ...
    # Windows builds
    WINDOWS_FILES_URL = "https://inla.r-inla-download.org/Windows-builds/FILES"
    WINDOWS_BASE_URL = "https://inla.r-inla-download.org/Windows-builds"
    ...
    def _fetch_windows_files_list(self) -> List[str]:
        """Fetch the FILES listing from R-INLA Windows builds server."""
        try:
            with urllib.request.urlopen(self.WINDOWS_FILES_URL, timeout=30) as response:
                content = response.read().decode('utf-8')
                return [line.strip() for line in content.split('\n') if line.strip()]
        except Exception as e:
            raise RuntimeError(f"Failed to fetch Windows builds list: {e}")

    def list_windows_binaries(self) -> List[Dict]:
        """List available Windows binaries.

        Returns:
            List of dicts with 'version', 'path', 'url' keys
        """
        files = self._fetch_windows_files_list()

        binaries = []
        for line in files:
            # Parse: ./windows64/Version_26.02.05/64bit.zip
            if not line.startswith('./') or not line.endswith('/64bit.zip'):
                continue
            ...
            if len(parts) >= 3:
                ...
                if ver_part.startswith('Version_'):
                    ...
                    # Build URL
                    url_path = urllib.parse.quote(line[2:], safe='/')
                    url = f"{self.WINDOWS_BASE_URL}/{url_path}"
    ...
    def download_windows_binary(
        ...
        Returns:
            Path to installed binary
        """
        binary_path = self.get_binary_path("windows")

        if not force and self.is_installed("windows"):
            print(f"INLA binary already installed at: {binary_path}")
            return binary_path

        # Get available binaries
        binaries = self.list_windows_binaries()

        if not binaries:
            raise RuntimeError("No Windows binaries found. Check your internet connection.")

        # Sort by version (newest first)
        binaries.sort(key=lambda x: x['version'], reverse=True)

        # Select version
        if version:
            filtered = [b for b in binaries if version in b['version']]
            if not filtered:
                available = [b['version'] for b in binaries]
                raise RuntimeError(
                    f"Version '{version}' not found.\n"
                    f"Available versions: {available[:5]}"
                )
            selected = filtered[0]
        else:
            selected = binaries[0]  # Latest

        print(f"\nSelected: Windows 64-bit Version {selected['version']}")

        # Download to temp file
        with tempfile.TemporaryDirectory() as tmpdir:
            zip_path = Path(tmpdir) / "64bit.zip"
            self._download_file(selected['url'], zip_path, "Downloading binary")
            ...
            if not inla_binary:
                raise RuntimeError("Could not find INLA binary in extracted files")

            # Install ALL files from extracted directory
            binary_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            for item in extracted_dir.iterdir():
                if item.is_file():
                    ...
                    # Copy subdirectories too (e.g., DLLs)
                    dst_dir = binary_path.parent / item.name
                    if dst_dir.exists():
                        shutil.rmtree(dst_dir)
                    shutil.copytree(item, dst_dir)
    ...
    def download_binary(
        ...
        Args:
            ...
            platform_name: Platform ("linux", "mac", "mac.arm64", "windows"). Auto-detected if None.
            force: Force re-download
        ...
        if platform_name == "linux":
            ...
            return self.download_windows_binary(version=version, force=force)
        else:
            raise ValueError(f"Unknown platform: {platform_name}")
...
def download_binary(
    ...
    Args:
        ...
        platform: Platform name: "linux", "mac", "mac.arm64", "windows" (auto-detected if None)
        force: Force re-download even if exists
    ...
    Example:
        ...
        >>> download_binary(platform="windows")  # Windows 64-bit
    """
...
def _cli_install():
    ...
    if args.list:
        ...
        if platform_name == "linux":
            ...
            print("\nWindows detected - binary auto-selected (64-bit).")
            print("\nAvailable versions:")
            binaries = _manager.list_windows_binaries()
            versions = sorted(set(b['version'] for b in binaries), reverse=True)
            for ver in versions[:5]:
                print(f"  - {ver}")
            print("\nJust run: pyinla-install")
        else:
    ...
    if args.list_versions:
        ...
        if platform_name == "linux":
            ...
            print("\nFetching available Windows binaries...")
            binaries = _manager.list_windows_binaries()
            versions = sorted(set(b['version'] for b in binaries), reverse=True)
