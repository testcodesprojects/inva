...
def sparse_det_bym(Q: np.ndarray,
    ...
    """
    Compute constrained log|Q| using the INLA binary (TAUCS/PARDISO).

    Matches R-INLA's inla.sparse.det.bym exactly by calling the same binary
    with ``-m qsample`` mode and ``sample=0``, then recovering logdet from
    the returned log-density:
        logdet = 2 * (logdens + (n - rankdef)/2 * log(2*pi))
    """
    Qd = _to_dense(Q).copy()  # copy to avoid modifying caller's matrix
    n = Qd.shape[0]
    if constr is None:
        ...
        if rankdef is None:
            rankdef = res["rankdef"]
    if rankdef is None:
        rankdef = constr["A"].shape[0] if constr is not None else 0

    # Regularise: diag(Q) += mean(diag) * eps  (same as R-INLA lines 64-65)
    d = np.diag(Qd)
    np.fill_diagonal(Qd, d + np.mean(d) * eps)

    # Call INLA binary via inla_qsample(sample=0) — same as R-INLA line 66
    from .sampling import inla_qsample
    sample_zero = np.zeros((n, 1), dtype=np.float64)
    result = inla_qsample(
        Q=Qd,
        sample=sample_zero,
        constr=constr,
    ...
    logdens = float(result["logdens"][0])
    logdet = 2.0 * (logdens + (n - rankdef) / 2.0 * math.log(2.0 * math.pi))
...
def pc_bym_phi(*,
    ...
    if use_eigs:
        ...
        # Run sequentially: each iteration calls inla_qsample (subprocess),
        # and concurrent subprocess.run with os.environ modifications is not
        # thread-safe — the _run_environment_set()/_unset() calls race.
