...
# Expiration date for this version (update when releasing new version)
_EXPIRATION_DATE = _datetime.date(2026, 6, 30)
...
def _check_expiration() -> None:
    ...
    if today > _EXPIRATION_DATE:
        raise RuntimeError(
            f"This was a testing version of pyINLA and it expired on {_EXPIRATION_DATE}. "
            f"Please update pyINLA to a stable version: pip install --upgrade pyinla"
        )
...
def _normalize_control_priors(control: Dict[str, Any]) -> None:
    ...
    if family_block is None:
        return
    # Support per-family list or single dict
    blocks = family_block if isinstance(family_block, list) else [family_block]
    for block in blocks:
        if not isinstance(block, dict):
            continue
        hyper = block.get("hyper")
    ...
    with ref_path.open("r", encoding="utf-8", errors="ignore") as fp:
        for raw in fp:
            ...
            if current == "data":
                ...
                elif line.startswith("id.names ="):
                    component["id.names"] = _extract_name(line)
                elif line.startswith("weights ="):
                    component["weights"] = _extract_name(line)
                elif line.startswith("extraconstraint ="):
                    component["extraconstraint"] = _extract_name(line)
...
def _resolve_reference_path(reuse_hint: Optional[str], working_directory: Optional[str]) -> Optional[str]:
    if reuse_hint:
        p = Path(reuse_hint)
        # If hint is a directory without Model.ini, check for inla.model subdir
        if p.is_dir() and not (p / "Model.ini").exists():
            inla_model = p / "inla.model"
            if inla_model.is_dir() and (inla_model / "Model.ini").exists():
                return str(inla_model)
        return reuse_hint
...
def _apply_control_defaults(control: Optional[Dict[str, Any]], primary_family: Optional[str] = None) -> Dict[str, Any]:
    ...
    # control.family: if user passed a list (per-family overrides for multiple
    # likelihoods), keep it as-is — defer merging with defaults to
    # _finalize_and_validate_families() where n_family is known.
    raw_family = ctrl.get("family", {})
    if isinstance(raw_family, list):
        ctrl["family"] = raw_family
    else:
    ...
    # Merge family hyper defaults for single-dict case only; list case is
    # handled per-family in _finalize_and_validate_families().
...
def _finalize_and_validate_families(fams: List[str],
    ...
    Mirror R's post-control family handling in one place:
      ...
      - Resolve control.family into a **list of dicts** (one per family),
        merging each with defaults and populating family-specific hyper.
      - Return the families (unchanged), possibly-updated control dict, and n_family.

    Supports per-family control.family overrides (R parity):
      - Single dict: replicated for each family.
      - List of dicts with len == n_family: used as-is (per-family overrides).
      - List with wrong length: raises an error.
    """
    _validate_families(fams)
    n_family = len(fams)
    fam_ctrl = control_resolved.get("family", {})

    # --- Normalize to a list-of-dicts of length n_family
    if isinstance(fam_ctrl, list):
        # User provided per-family list — validate length
        if len(fam_ctrl) != n_family:
            raise PyINLAError(
                f"Argument 'control.family' has length {len(fam_ctrl)} but "
                f"length(family) = {n_family}. They must match."
            )
        family_list = [_normalize_keys(fc) if isinstance(fc, dict) else {} for fc in fam_ctrl]
    else:
        # Single dict (already merged with defaults by _apply_control_defaults)
        # or empty — replicate for each family via deep copy.
        base = fam_ctrl if isinstance(fam_ctrl, dict) else {}
        family_list = [copy.deepcopy(base) for _ in range(n_family)]

    # --- Per-family: merge with defaults and populate hyper
    for i, fam in enumerate(fams):
        entry = family_list[i]
        # If this entry came from a per-family list, it hasn't been merged
        # with defaults yet (skipped in _apply_control_defaults).
        if isinstance(fam_ctrl, list):
            entry = _recursive_merge(control_family_default(), entry)
        try:
            _merge_family_hyper_defaults(entry, fam)
        except Exception:
            pass
        family_list[i] = entry

    control_resolved["family"] = family_list
    return fams, control_resolved, n_family
...
def _core(*,
    ...
    for i in range(n_family):
        ...
        # Per-family control: control_resolved["family"] is now a list of dicts
        family_ctrl_list = control_resolved.get("family", [{}])
        family_ctrl_i = family_ctrl_list[i] if isinstance(family_ctrl_list, list) else family_ctrl_list
    ...
    if len(random_specs) > 0:
        ...
        for ridx, r in enumerate(random_specs, start=1):
            ...
            is_bym_model = model_name.lower() == "bym"
            ...
            # --- Random effect weights file (like f(Subject, Days, model="iid") in R) ---
            # R-INLA format: 2-column matrix [0-based obs index, weight value]
            file_weights_path = None
            weights_spec = r.get("weights")
            if weights_spec is not None:
                if isinstance(weights_spec, str):
                    if weights_spec not in df.columns:
                        raise PyINLAError(
                            f"Random component '{r.get('id', ridx)}': "
                            f"weights column '{weights_spec}' not found in data."
                        )
                    series = df[weights_spec]
                    if pd is not None and hasattr(series, "to_numpy"):
                        w_values = pd.to_numeric(series, errors="coerce").to_numpy(dtype=float)
                    else:
                        w_values = np.asarray(series, dtype=float)
                else:
                    w_values = np.asarray(weights_spec, dtype=float).ravel()

                if w_values.size != n:
                    raise PyINLAError(
                        f"Random component '{r.get('id', ridx)}': "
                        f"weights length mismatch (expected {n}, got {w_values.size})."
                    )

                from pyinla.safety import enforce_random_weights_values
                enforce_random_weights_values(w_values, r.get("id", f"rand{ridx}"))

                # Replace NaN with 0 (matching R-INLA: www[is.na(www)] <- 0)
                w_values = np.where(np.isfinite(w_values), w_values, 0.0)

                indN = np.arange(n, dtype=np.float64)
                weights_matrix = np.column_stack([indN, w_values])

                ref_weights = (random_refs[ridx-1].get("weights")
                               if ridx-1 < len(random_refs) else None)
                weights_name = ref_weights or f"rand-{ridx}-weights.dat"
                weights_abs = os.path.join(data_dir, weights_name)
                write_fmesher_file(weights_matrix, weights_abs)
                file_weights_path = weights_abs.replace(data_dir_abs, "$inladatadir")

            # --- id.names: write factor level names (R parity) ---
            # Priority: (1) user-provided, (2) copy from reference, (3) auto-detect.
            # Copying from reference is preferred over auto-detect because rpy2
            # converts R factors to pandas Categoricals with integer categories,
            # so auto-detect would write [1,2,3,...] instead of the original
            # factor level names like ['Blend1','Blend2',...].
            file_id_names_path = None
            ref_id_names = random_refs[ridx-1].get("id.names") if ridx-1 < len(random_refs) else None
            user_id_names = r.get("id_names")
            if user_id_names is not None:
                # (1) User provided explicit level names
                try:
                    from pyinla.sections import write_id_names_file
                    names_list = list(user_id_names)
                    id_names_name = ref_id_names or f"rand-{ridx}-id-names.dat"
                    id_names_abs = os.path.join(data_dir, id_names_name)
                    write_id_names_file(names_list, id_names_abs)
                    file_id_names_path = id_names_abs.replace(data_dir_abs, "$inladatadir")
                except Exception:
                    file_id_names_path = None
            elif ref_id_names and reference_path:
                # (2) Copy from R-INLA reference — preserves original factor names
                import shutil as _shutil
                ref_data_dir = os.path.join(reference_path, "data.files")
                ref_file = os.path.join(ref_data_dir, ref_id_names)
                if os.path.isfile(ref_file):
                    try:
                        id_names_name = ref_id_names
                        id_names_abs = os.path.join(data_dir, id_names_name)
                        _shutil.copy2(ref_file, id_names_abs)
                        file_id_names_path = id_names_abs.replace(data_dir_abs, "$inladatadir")
                    except Exception:
                        file_id_names_path = None
            else:
                # (3) Auto-detect from dataframe column
                names_list = None
                if (isinstance(id_obj, str) and id_obj in df.columns
                        and hasattr(df[id_obj], "cat")):
                    try:
                        names_list = [str(c) for c in df[id_obj].cat.categories]
                    except Exception:
                        names_list = None
                elif (isinstance(id_obj, str) and id_obj in df.columns
                      and df[id_obj].dtype == object):
                    try:
                        names_list = sorted(df[id_obj].dropna().unique().tolist())
                    except Exception:
                        names_list = None
                if names_list:
                    try:
                        from pyinla.sections import write_id_names_file
                        id_names_name = f"rand-{ridx}-id-names.dat"
                        id_names_abs = os.path.join(data_dir, id_names_name)
                        write_id_names_file(names_list, id_names_abs)
                        file_id_names_path = id_names_abs.replace(data_dir_abs, "$inladatadir")
                    except Exception:
            ...
            # Use aug_factor from model_props for all models (e.g., generic2 has aug_factor=2)
            aug_factor = int(model_props.get("aug_factor", 1))
            ...
            # R-INLA uses one unified constraint logic for besag, besag2, bym, bym2:
            # one sum-to-zero row per connected component with size >= 2.
            # Singletons are excluded (they get an improper prior).
            # For augmented models (bym/bym2), constr_offset places constraints
            # on the second half of the field (aug.constr = 2).
            if (
            ...
            # R-INLA inla.R lines 1889-1934: For augmented spatial models (bym, bym2, besag, etc.)
            # with aug_factor > 1, the constraint is converted to extraconstr.
            # The constraint is placed on the portion of the latent field defined by aug_constr.
            # For BYM2: aug_factor=2, aug_constr=2 means constraint on second half.
            # This runs AFTER multi-component adjustment (which matches R-INLA's flow:
            # f.R handles multi-component first, then inla.R handles augmented models).
            # NOTE: This only applies to spatial models in adjust_models (besag, bym, bym2, besag2).
            # Other augmented models like generic2 use constraint=1 directly without extraconstraint.
            if (
                model_lower in adjust_models
                and model_props.get("augmented")
                and aug_factor > 1
                and r.get("constr", model_props.get("constr", False))
                and not force_constr_off  # Don't duplicate if already handled (multi-component, iidkd, etc.)
            ):
                # Build A matrix: size (1, aug_factor * n)
                # Place 1s in positions [(aug_constr-1)*n : aug_constr*n]
                A_aug = np.zeros((1, latent_dim), dtype=float)
                start_idx = constr_offset  # = (aug_constr_val - 1) * component_n
                end_idx = start_idx + component_n
                A_aug[0, start_idx:end_idx] = 1.0
                extra_constr = _stack_extra_constraints(extra_constr, A_aug, np.array([0.0]), latent_dim)
                force_constr_off = True
            ...
            if graph_obj is not None and model_lower in adjust_models and r.get("rankdef") is None:
                # R-INLA computes rankdef = base + user_extraconstr_rows.
                # The per-component constraints added internally by adjust.for.con.comp
                # are NOT counted in rankdef (they are added after rankdef is set).
                # Only user-provided extraconstr rows (from the extraconstr argument)
                # are counted, matching R-INLA's f.R lines 909-917.
                user_extra_rows = 0
                if extraconstr is not None and extra_constr is not None:
                    # User provided extraconstr; count those rows only
                    user_A = extraconstr.get("A")
                    if user_A is not None:
                        user_A_arr = np.asarray(user_A)
                        user_extra_rows = 1 if user_A_arr.ndim == 1 else user_A_arr.shape[0]
                r = r.copy()
                base_rankdef = cc_n2 if scale_model_flag else cc_n2 + cc_n1
                r["rankdef"] = int(base_rankdef + user_extra_rows)
            ...
            if force_constr_off:
                ...
                # When extraconstraint exists (either computed or from reference),
                # constraint must be 0 (False) since the constraint is managed via extraconstraint.
                # This matches R-INLA behavior where constraint=0 when extraconstraint is present.
                random_spec_for_section = r.copy()
                random_spec_for_section["constr"] = False
                if random_spec_for_section.get("diagonal") is None:
                    random_spec_for_section["diagonal"] = 1e-04
        ...
        if collect_results:
            ...
            try:
                ...
                if isinstance(results_dict, dict):
                    ...
                    _slcd = results_dict.get("summary.lincomb.derived")
                    summary_lincomb = _slcd if _slcd is not None else results_dict.get("summary.lincomb")
                    _mlcd = results_dict.get("marginals.lincomb.derived")
                    marginals_lincomb = _mlcd if _mlcd is not None else results_dict.get("marginals.lincomb")
...
class _PyINLAInterface:
    ...
    def __call__(self, **kwargs) -> PyINLAResult:
        ...
        try:
            ...
            enforce_multiple_likelihood_response(kwargs, families=families)
