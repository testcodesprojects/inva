...
# Letters, digits, whitespace, arithmetic, comparison, parentheses, semicolons,
# assignment, decimal point, sign, comma, caret (power in muparser).
...
def _get_response_object(kwargs: Dict[str, Any]) -> Optional[Any]:
    ...
    if isinstance(response, list) and all(isinstance(r, str) for r in response):
        # Multiple likelihoods: response is a list of column names.
        # Per-family validation is not possible here because each column
        # corresponds to a different family and we cannot map them at
        # this stage.  Skip response-value validation; R-INLA will
        # validate each family's data internally.
        return None
...
def enforce_multiple_likelihood_response(
    ...
    For multiple likelihoods, each observation (row) should have a non-NaN value
    in exactly one response column. Overlapping observations (same row has values
    in multiple response columns) will cause INLA to fail with
    "Observation X occurs in more than one data-section".

    This check catches this issue early with a clear error message.
    """
    import numpy as np
    ...
    # Only validate for multiple likelihoods (list of column names)
    ...
    # Check that all response columns exist
    ...
    # Extract response columns and check NaN pattern
    n_rows = len(data)
    response_arrays = []
    for col in response:
    ...
    # Note: R-INLA allows a response column to have all NaN values (meaning that
    # likelihood has no observations). We don't reject this case to match R-INLA behavior.
...
def enforce_random_structure(kwargs: Dict[str, Any]) -> None:
    ...
    # 'id_names' is optional on all models: provides factor level names for R-INLA parity
...
def _validate_single_family_control(family_name: str, family_block: Dict[str, Any], idx: Optional[int] = None) -> None:
    ...
    ----------
    family_name : str
        The likelihood name (e.g., 'gaussian', 'poisson').
    family_block : dict
        The control.family dict to validate.
    idx : int, optional
        Index for error messages when validating a list of controls.
    """
    prefix = f"control['family'][{idx}]" if idx is not None else "control['family']"

    # Determine what this specific family supports
    ...
    # Build allowed keys for this specific family
    allowed_keys = set()
    ...
    # Validate 'beta.censor.value' if present
...
def enforce_binomial_family_variant(
    ...
    # Handle list of dicts for per-family control overrides (multiple likelihoods)
    if isinstance(family_block, list):
        ...
        # Validate each control against its corresponding family
        ...
        for idx, (fam, fb) in enumerate(zip(normalized, family_block)):
            if fb:  # Only validate non-empty dicts
    ...
    # Single dict: validate against all families (original behavior for backward compat)
    # This handles the case where a single control.family is broadcast to all families
...
def enforce_untested_arguments(kwargs: Dict[str, Any]) -> None:
    ...
    # NOTE: verbose, num_threads, keep, working_directory, reuse_filenames_from,
    # and inla_call are allowed as they are infrastructure parameters that don't
    # affect model specification or input file generation.
    untested_execution = {
        "silent": "silent mode",
