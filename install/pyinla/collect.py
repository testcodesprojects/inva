...
def _collect_misc_configs_preopt(d: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with open(fnm, "rb") as fp:
            ...
            # Calculate config size before the loop
            config_start_pos = fp.tell()
            fp.seek(0, 2)  # End of file
            file_end = fp.tell()
            fp.seek(config_start_pos)  # Return
            total_config_data = file_end - config_start_pos
            config_size_bytes = total_config_data // nconfig if nconfig > 0 else 0

            for cfg_idx in range(nconfig):
                # Record position for this config
                this_config_start = fp.tell()
                ...
                # Optional fields - format varies by INLA version
                arg_str = None
                ll_info = None
                AP_mv = np.empty((0, 2))
                P_mv = np.empty((0, 2))

                # Try reading predictor data
                try:
                    have_lpred_val = _read_float64s(fp, 1)
                    if have_lpred_val.size > 0 and have_lpred_val[0] == 1.0:
                ...
                # Seek to next config position (handles variable-length optional data)
                next_config_pos = this_config_start + config_size_bytes
                fp.seek(next_config_pos)
